...
### v2.3.0 (2026-02-19) — CRAN Compliance

- Fixed CRAN submission issues from Benjamin Altmann review:
  - Single-quoted software names ('Python' and 'Stata')
  - Added acronym explanations (SDMX, SDG)
  - Cache functions now use `tempdir()` by default; persistent caching requires explicit opt-in via environment variables
- Refactored `set_metadata_cache()` and `.get_cache_path()` to avoid writing to home directory without permission
