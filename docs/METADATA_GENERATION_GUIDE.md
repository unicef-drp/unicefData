...
│   └── dataflows/                      # Individual dataflow schema files (69)
│       └── ...
└── stata/src/_/                             # Co-located with ado files (adopath-resolved)
    ├── _unicefdata_dataflows.yaml           # Dataflow definitions
    ├── _unicefdata_codelists.yaml           # Valid dimension codes
    ├── _unicefdata_countries.yaml           # Country ISO3 codes
    ├── _unicefdata_regions.yaml             # Regional aggregate codes
    ├── _unicefdata_indicators.yaml          # Indicator → dataflow mappings
    ├── _unicefdata_indicators_metadata.yaml # Full indicator codelist (748 items)
    ├── _unicefdata_dataflow_metadata.yaml   # Dataflow dimensions and attributes
    ├── _indicator_dataflow_map.yaml         # Indicator to dataflow mapping
    ├── _dataflow_index.yaml                 # Index of dataflow schemas
    ├── _dataflow_fallback_sequences.yaml    # Fallback dataflow order
    └── _dataflows/                          # Individual dataflow schemas (70)
        └── ...
```
...
cd "python"
python -c "from unicefdata.schema_sync import sync_dataflow_schemas; sync_dataflow_schemas(output_dir='../stata/src/_')"
```
...
# Clean Stata
Remove-Item "stata/src/_/*.yaml" -Force -ErrorAction SilentlyContinue
Remove-Item "stata/src/_/_dataflows/*.yaml" -Force -ErrorAction SilentlyContinue
```
...
# Clean Stata
rm -f stata/src/_/*.yaml
rm -f stata/src/_/_dataflows/*.yaml
```
...
- **Stata**: Files in `stata/src/_/` (co-located with ado files, adopath-resolved at runtime)
...
(Get-ChildItem "stata/src/_/_unicefdata_*.yaml").Count  # Should be 7+
Test-Path "stata/src/_/_unicefdata_indicators_metadata.yaml"  # Should be True
(Get-ChildItem "stata/src/_/_dataflows/*.yaml").Count  # Should be 70

# Check Stata (stataonly files, if generated)
(Get-ChildItem "stata/src/_/*_stataonly.yaml").Count  # Should be 7
```
...
4. **Macro substitution too long (error 920)**: This occurs when parsing large XML files with pure Stata. Use Python for dataflow schemas:
   ...
   cd "python"
   shell python -c "from unicef_api.schema_sync import sync_dataflow_schemas; sync_dataflow_schemas(output_dir='../stata/src/_')"
   ```
