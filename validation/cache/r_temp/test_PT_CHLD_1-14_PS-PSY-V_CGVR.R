...
tryCatch({
    df <- unicefData(indicator = "PT_CHLD_1-14_PS-PSY-V_CGVR", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/PT_CHLD_1-14_PS-PSY-V_CGVR.csv", row.names = FALSE, fileEncoding = "UTF-8")
