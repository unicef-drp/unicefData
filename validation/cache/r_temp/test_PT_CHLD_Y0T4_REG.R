...
tryCatch({
    df <- unicefData(indicator = "PT_CHLD_Y0T4_REG", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/PT_CHLD_Y0T4_REG.csv", row.names = FALSE, fileEncoding = "UTF-8")
