...
tryCatch({
    df <- unicefData(indicator = "PT_CHLD_5-17_LBR_ECON", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/PT_CHLD_5-17_LBR_ECON.csv", row.names = FALSE, fileEncoding = "UTF-8")
