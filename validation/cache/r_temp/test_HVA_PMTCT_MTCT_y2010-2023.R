...
tryCatch({
    df <- unicefData(indicator = "HVA_PMTCT_MTCT", metadata = "light", year = "2010:2023")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/HVA_PMTCT_MTCT_y2010-2023.csv", row.names = FALSE, fileEncoding = "UTF-8")
