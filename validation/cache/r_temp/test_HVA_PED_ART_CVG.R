...
tryCatch({
    df <- unicefData(indicator = "HVA_PED_ART_CVG", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/HVA_PED_ART_CVG.csv", row.names = FALSE, fileEncoding = "UTF-8")
