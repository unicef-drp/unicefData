...
tryCatch({
    df <- unicefData(indicator = "ECD_CHLD_U5_PLYTH-HM", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/ECD_CHLD_U5_PLYTH-HM.csv", row.names = FALSE, fileEncoding = "UTF-8")
