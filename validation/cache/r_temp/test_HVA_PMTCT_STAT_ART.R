...
tryCatch({
    df <- unicefData(indicator = "HVA_PMTCT_STAT_ART", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/HVA_PMTCT_STAT_ART.csv", row.names = FALSE, fileEncoding = "UTF-8")
