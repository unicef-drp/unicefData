...
tryCatch({
    df <- unicefData(indicator = "SPP_GINI", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/SPP_GINI.csv", row.names = FALSE, fileEncoding = "UTF-8")
