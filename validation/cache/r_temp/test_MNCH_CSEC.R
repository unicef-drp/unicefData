...
tryCatch({
    df <- unicefData(indicator = "MNCH_CSEC", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/MNCH_CSEC.csv", row.names = FALSE, fileEncoding = "UTF-8")
