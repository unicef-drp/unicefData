...
tryCatch({
    df <- unicefData(indicator = "DM_FRATE_TOT", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/DM_FRATE_TOT.csv", row.names = FALSE, fileEncoding = "UTF-8")
