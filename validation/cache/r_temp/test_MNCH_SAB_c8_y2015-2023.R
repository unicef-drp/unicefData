...
tryCatch({
    df <- unicefData(indicator = "MNCH_SAB", metadata = "light", countries = c("AFG", "BGD", "COD", "ETH", "IND", "IDN", "NGA", "PAK"), year = "2015:2023")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/MNCH_SAB_c8_y2015-2023.csv", row.names = FALSE, fileEncoding = "UTF-8")
