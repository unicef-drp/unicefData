...
tryCatch({
    df <- unicefData(indicator = "PT_BRTH_RGSTR", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/PT_BRTH_RGSTR.csv", row.names = FALSE, fileEncoding = "UTF-8")
