...
tryCatch({
    df <- unicefData(indicator = "DM_POP_U5", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/DM_POP_U5.csv", row.names = FALSE, fileEncoding = "UTF-8")
