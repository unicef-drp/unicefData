...
tryCatch({
    df <- unicefData(indicator = "HVA_EPI_INF_RT_0-14", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/HVA_EPI_INF_RT_0-14.csv", row.names = FALSE, fileEncoding = "UTF-8")
