...
tryCatch({
    df <- unicefData(indicator = "ECON_GVT_EXP_PTGDP", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/ECON_GVT_EXP_PTGDP.csv", row.names = FALSE, fileEncoding = "UTF-8")
