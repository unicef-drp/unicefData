...
tryCatch({
    df <- unicefData(indicator = "SPP_CHLD_POV_PROF", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/SPP_CHLD_POV_PROF.csv", row.names = FALSE, fileEncoding = "UTF-8")
