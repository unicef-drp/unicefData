...
tryCatch({
    df <- unicefData(indicator = "GN_FB_BNK_ACCSS", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/GN_FB_BNK_ACCSS.csv", row.names = FALSE, fileEncoding = "UTF-8")
