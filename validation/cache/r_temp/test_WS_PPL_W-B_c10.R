...
tryCatch({
    df <- unicefData(indicator = "WS_PPL_W-B", metadata = "light", countries = c("AFG", "BGD", "ETH", "KEN", "MMR", "NGA", "PAK", "TZA", "UGA", "ZMB"))

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/WS_PPL_W-B_c10.csv", row.names = FALSE, fileEncoding = "UTF-8")
