...
tryCatch({
    df <- unicefData(indicator = "IM_MCV1", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/IM_MCV1.csv", row.names = FALSE, fileEncoding = "UTF-8")
