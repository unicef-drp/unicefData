...
tryCatch({
    df <- unicefData(indicator = "HVA_EPI_LHIV_15-24", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/HVA_EPI_LHIV_15-24.csv", row.names = FALSE, fileEncoding = "UTF-8")
