...
tryCatch({
    df <- unicefData(indicator = "SPP_CHLD_SOC_PROT", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/SPP_CHLD_SOC_PROT.csv", row.names = FALSE, fileEncoding = "UTF-8")
