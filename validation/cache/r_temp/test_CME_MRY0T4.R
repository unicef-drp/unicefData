...
tryCatch({
    df <- unicefData(indicator = "CME_MRY0T4", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/CME_MRY0T4.csv", row.names = FALSE, fileEncoding = "UTF-8")
