...
tryCatch({
    df <- unicefData(indicator = "NT_ANT_HAZ_NE2_nofilter", metadata = "light", year = "2018:2023", raw = TRUE)

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/NT_ANT_HAZ_NE2_nofilter_y2018-2023_nofilter.csv", row.names = FALSE, fileEncoding = "UTF-8")
