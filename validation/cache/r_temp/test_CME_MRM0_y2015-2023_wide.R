...
tryCatch({
    df <- unicefData(indicator = "CME_MRM0", metadata = "light", year = "2015:2023", format = "wide")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/CME_MRM0_y2015-2023_wide.csv", row.names = FALSE, fileEncoding = "UTF-8")
