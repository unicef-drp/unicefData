...
tryCatch({
    df <- unicefData(indicator = "IM_HEPB3", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/IM_HEPB3.csv", row.names = FALSE, fileEncoding = "UTF-8")
