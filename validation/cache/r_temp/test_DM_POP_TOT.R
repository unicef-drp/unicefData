...
tryCatch({
    df <- unicefData(indicator = "DM_POP_TOT", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/DM_POP_TOT.csv", row.names = FALSE, fileEncoding = "UTF-8")
