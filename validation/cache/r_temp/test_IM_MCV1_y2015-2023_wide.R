...
tryCatch({
    df <- unicefData(indicator = "IM_MCV1", metadata = "light", year = "2015:2023", format = "wide")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/IM_MCV1_y2015-2023_wide.csv", row.names = FALSE, fileEncoding = "UTF-8")
