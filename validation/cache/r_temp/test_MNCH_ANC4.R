...
tryCatch({
    df <- unicefData(indicator = "MNCH_ANC4", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/MNCH_ANC4.csv", row.names = FALSE, fileEncoding = "UTF-8")
