...
tryCatch({
    df <- unicefData(indicator = "PV_CHLD_INCM-PL", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/PV_CHLD_INCM-PL.csv", row.names = FALSE, fileEncoding = "UTF-8")
