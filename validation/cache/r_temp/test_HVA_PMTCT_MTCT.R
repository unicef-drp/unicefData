...
tryCatch({
    df <- unicefData(indicator = "HVA_PMTCT_MTCT", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/HVA_PMTCT_MTCT.csv", row.names = FALSE, fileEncoding = "UTF-8")
