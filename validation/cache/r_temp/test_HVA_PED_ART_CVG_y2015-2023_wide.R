...
tryCatch({
    df <- unicefData(indicator = "HVA_PED_ART_CVG", metadata = "light", year = "2015:2023", format = "wide")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/HVA_PED_ART_CVG_y2015-2023_wide.csv", row.names = FALSE, fileEncoding = "UTF-8")
