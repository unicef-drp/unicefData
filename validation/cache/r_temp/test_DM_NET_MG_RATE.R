...
tryCatch({
    df <- unicefData(indicator = "DM_NET_MG_RATE", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/DM_NET_MG_RATE.csv", row.names = FALSE, fileEncoding = "UTF-8")
