...
tryCatch({
    df <- unicefData(indicator = "CME_MRY5T9", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/CME_MRY5T9.csv", row.names = FALSE, fileEncoding = "UTF-8")
