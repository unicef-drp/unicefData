...
tryCatch({
    df <- unicefData(indicator = "CME_MRY0T4", metadata = "light", year = "2018:2023", raw = TRUE)

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/CME_MRY0T4_y2018-2023_nofilter.csv", row.names = FALSE, fileEncoding = "UTF-8")
