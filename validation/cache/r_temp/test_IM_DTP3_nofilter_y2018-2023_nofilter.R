...
tryCatch({
    df <- unicefData(indicator = "IM_DTP3_nofilter", metadata = "light", year = "2018:2023", raw = TRUE)

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/IM_DTP3_nofilter_y2018-2023_nofilter.csv", row.names = FALSE, fileEncoding = "UTF-8")
