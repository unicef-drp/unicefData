...
tryCatch({
    df <- unicefData(indicator = "ECD_CHLD_24-59M_ADLT_SRC", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/ECD_CHLD_24-59M_ADLT_SRC.csv", row.names = FALSE, fileEncoding = "UTF-8")
