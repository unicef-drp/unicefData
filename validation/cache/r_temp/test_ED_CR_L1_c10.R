...
tryCatch({
    df <- unicefData(indicator = "ED_CR_L1", metadata = "light", countries = c("USA", "GBR", "BRA", "IND", "NGA", "ZAF", "CHN", "JPN", "DEU", "FRA"))

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/ED_CR_L1_c10.csv", row.names = FALSE, fileEncoding = "UTF-8")
