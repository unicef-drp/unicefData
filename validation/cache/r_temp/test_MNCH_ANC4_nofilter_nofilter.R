...
tryCatch({
    df <- unicefData(indicator = "MNCH_ANC4_nofilter", metadata = "light", raw = TRUE)

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/MNCH_ANC4_nofilter_nofilter.csv", row.names = FALSE, fileEncoding = "UTF-8")
