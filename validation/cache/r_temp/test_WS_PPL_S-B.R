...
tryCatch({
    df <- unicefData(indicator = "WS_PPL_S-B", metadata = "light")

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/WS_PPL_S-B.csv", row.names = FALSE, fileEncoding = "UTF-8")
