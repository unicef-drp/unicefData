...
tryCatch({
    df <- unicefData(indicator = "PT_F_20-24_MRD_U18_nofilter", metadata = "light", raw = TRUE)

    if (nrow(df) > 0) {
        write.csv(df, "C:/GitHub/myados/unicefData-dev/validation/cache/r/PT_F_20-24_MRD_U18_nofilter_nofilter.csv", row.names = FALSE, fileEncoding = "UTF-8")
