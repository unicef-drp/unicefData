...
│   └── ...
│
└── stata/metadata/current/        ← Platform copy (DIFFERENT FORMAT)
    ├── dataflows/
    ├── _unicefdata_dataflows.yaml
    └── ...
```
...
| stata/metadata/current/ | _unicefdata_dataflows.yaml | 2025-12-05 07:53 UTC | ⚠️ Outdated (1+ months old) |
...
stata/metadata/current/_dataflow_fallback_sequences.yaml  (converted format)
```
...
cp metadata/current/_dataflow_fallback_sequences.yaml stata/metadata/current/
```
...
- **Day 3**: Document v1.6.1 release with unified fallback logic
...
| `stata/metadata/current/_dataflow_fallback_sequences.yaml` | SYNC | MEDIUM |
