...
Usage:
    ...
    # For highest accuracy, use cache-based validation:
    sampler_cached = ValidIndicatorSampler(use_cache_validation=True)
    valid_indicators = sampler_cached.filter_valid_indicators(all_indicators)
"""
...
# =============================================================================
# Cache-based Validator Class
# =============================================================================

class CacheBasedValidator:
    """
    Validate indicators against actual cached data.
    
    This uses indicators that have successfully returned data as the source
    of truth, rather than naming conventions alone.
    """
    ...
    def __init__(self, cache_dir: str = None):
        ...
        ----------
        cache_dir : str
            Path to cache directory with indicator CSV files.
            Default: validation/cache/python/
        """
        if cache_dir is None:
            # Default to Python cache (most complete)
            script_dir = Path(__file__).parent
            repo_root = script_dir.parent.parent
            cache_dir = repo_root / "validation" / "cache" / "python"
        
        self.cache_dir = Path(cache_dir)
        self._cached_indicators = None
    
    def get_cached_indicators(self) -> Set[str]:
        """Get set of indicators that have successful cached data"""
        if self._cached_indicators is None:
            self._cached_indicators = set()
            if self.cache_dir.exists():
                for f in self.cache_dir.glob("*.csv"):
                    # Filename is INDICATOR_CODE.csv
                    indicator_code = f.stem
                    self._cached_indicators.add(indicator_code)
            logger.info(f"Found {len(self._cached_indicators)} cached indicators in {self.cache_dir}")
        return self._cached_indicators
    
    def is_valid_indicator(self, code: str) -> Tuple[bool, str]:
        """Check if indicator exists in cache (proven to work)"""
        cached = self.get_cached_indicators()
        if code in cached:
            return True, "Cached (proven valid)"
        return False, "Not in cache (may not exist)"
    
    def filter_to_cached(self, indicators: Dict[str, any]) -> Dict[str, any]:
        """Filter indicators to only those with cached data"""
        cached = self.get_cached_indicators()
        valid = {code: meta for code, meta in indicators.items() if code in cached}
        logger.info(f"Filtered {len(indicators)} → {len(valid)} indicators (cached only)")
        return valid
...
class ValidIndicatorSampler:
    ...
    def __init__(self, allow_unknown_prefixes: bool = False, verbose: bool = True,
                 use_cache_validation: bool = False):
        """
        Parameters
        ----------
        allow_unknown_prefixes : bool
        ...
        use_cache_validation : bool
            If True, use cache-based validation (most accurate but requires prior runs)
        """
        self.validator = IndicatorValidator(allow_unknown_prefixes)
        self.cache_validator = CacheBasedValidator() if use_cache_validation else None
        self.use_cache_validation = use_cache_validation
    ...
    def filter_valid_indicators(self, indicator_dict: Dict[str, any]) -> Dict[str, any]:
        ...
        Uses cache-based validation if use_cache_validation=True (more accurate),
        otherwise falls back to naming convention heuristics.
        ...
        """
        # Use cache-based validation if enabled (most accurate)
        if self.use_cache_validation and self.cache_validator:
            cached_indicators = self.cache_validator.filter_to_cached(indicator_dict)
            if len(cached_indicators) > 0:
                if self.verbose:
                    logger.info(f"Using cache-based validation: {len(cached_indicators)} indicators with cached data")
                return cached_indicators
            else:
                logger.warning("No cached indicators found, falling back to naming convention validation")
        
        # Fall back to naming convention heuristics
        valid = {}
        invalid = []
