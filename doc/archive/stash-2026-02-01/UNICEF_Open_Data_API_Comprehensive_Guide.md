# UNICEF Open Data APIs ΓÇö Comprehensive Developer Guide
...
3. [SDMX REST API ΓÇö UNICEF Data Warehouse](#3-sdmx-rest-api--unicef-data-warehouse)
...
This document provides a **complete, productionΓÇæready guide** to accessing UNICEF open data programmatically.  
It consolidates and crossΓÇæreferences:

- **UNICEF Open Data Portal** (humanΓÇæreadable datasets & metadata)
...
### 2.2 HumanΓÇæReadable Access (data.unicef.org)
...
SDMX does **not** support named parameters ΓÇö order is mandatory.
...
Need aggregation?
 Γö£ΓöÇ Yes ΓåÆ _T
 ΓööΓöÇ No ΓåÆ specify codes

Multiple indicators?
 Γö£ΓöÇ Yes ΓåÆ +
 ΓööΓöÇ No ΓåÆ single code

Production pipeline?
 Γö£ΓöÇ Yes ΓåÆ labels=id
 ΓööΓöÇ No ΓåÆ labels=both
```
...
### Child Poverty ΓÇö multiple indicators
...
### Nutrition ΓÇö aggregate totals
...
- ~~AutoΓÇægenerate query templates from DSDs~~ Γ£à Done via `unicefdata, info()`
- ~~Build a queryΓÇæbuilder utility (R / Python / Stata)~~ Γ£à Done via `unicefdata` package
- Add contract tests for metadata drift
- Maintain a perΓÇædataflow query cookbook
- Integrate with validation frameworks for data quality checks

---

*This document is intended to serve as a stable technical reference for UNICEF SDMXΓÇæbased analytics and data engineering pipelines.*
