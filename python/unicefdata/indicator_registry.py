...
Usage:
    ...
    >>> from unicefdata.indicator_registry import refresh_indicator_cache
...
def _get_cache_path() -> Path:
    ...
    # 2. Bundled metadata inside the package (unicefdata/metadata/current/)
    package_dir = Path(__file__).parent  # unicefdata/
    bundled_dir = package_dir / "metadata" / "current"
    if bundled_dir.exists():
        return bundled_dir / CACHE_FILENAME

    # 3. Repo/dev path (legacy: python/metadata/current/)
...
def _infer_category(indicator_code: str) -> str:
    ...
    # This approach is maintained in unicefdata.py's _load_fallback_sequences()
    # =========================================================================

    import importlib
    _unicefdata_mod = importlib.import_module('unicefdata.unicefdata')

    # Load fallback sequences (cached after first use)
    fallback_sequences = getattr(_unicefdata_mod, 'FALLBACK_SEQUENCES', {})
    if not fallback_sequences:
        fallback_sequences = _unicefdata_mod._load_fallback_sequences()
